...
(function(factory) {
    ...
    (function(H) {
        ...
        H.Chart.prototype.addKeyboardNavEvents = function() {
            ...
            // The model's keyCode handlers determine when to move to another model.
            // Validate holds a function to determine if there are prerequisites for this model to run that are not met.
            ...
            function navModuleFactory(keyMap, options) {
                return new KeyboardNavigationModule(merge({
                    keyCodeMap: keyMap,
                    // Move to next/prev valid model, or undefined if none, and init it.
                    // Returns true on success and false if there is no valid model to move to.
                    move: function(direction) {
                        ...
                        if (newModule) {
                            if (newModule.validate && !newModule.validate()) {
                                return this.move(direction); // Invalid model
                            }
                            if (newModule.init) {
                                newModule.init(direction); // Valid model, init it
                                return true;
                            }
                        }
                        // No model
            ...
            function keydownHandler(ev) {
                ...
                // If there is a navigation model for the current index, run it. Otherwise, we are outside of the chart in some direction.
                if (curNavModule) {
            ...
            chart.keyboardNavigationModules = [
                ...
                navModuleFactory([
                    ...
                    [
                        ...
                        function(keyCode) {
                            if (!chart.highlightAdjacentPoint(keyCode === 39)) { // Try to highlight adjacent point
                                return this.move(keyCode === 39 ? 1 : -1); // Failed. Move to next/prev model
                            }
                        }
                    ],
                    // Up/Down
                        ...
                        function(keyCode) {
                            ...
                            if (chart.highlightedPoint) {
                                ...
                                if (newSeries && newSeries.points[0]) { // If series exists and has data, go for it
                                    ...
                                    return this.move(keyCode === 40 ? 1 : -1); // Otherwise, attempt to move to next/prev model
                                }
                            }
                        }
                    ],
                    ...
                    // If coming back to points from other model, highlight last point
                ...
                navModuleFactory([
                    ...
                    [
                        ...
                        function() {
                            ...
                            if (reachedEnd) {
                                ...
                                // Try to move to prev model (should be points, since we wrapped to last point)
                                return this.move(-1);
                            }
                        }
                    ],
                    ...
                    init: function(direction) {
                        ...
                        // If coming back to export menu from other model, try to highlight last item in menu
                ...
                navModuleFactory([
                    ...
                    [
                        ...
                        function(keyCode, e) {
                            ...
                            if (e.shiftKey && !chart.focusedMapNavButtonIx || !e.shiftKey && chart.focusedMapNavButtonIx) { // trying to go somewhere we can't?
                                ...
                                return this.move(e.shiftKey ? -1 : 1); // Nowhere to go, go to prev/next model
                            }
                    ...
                    // Only run this model if we have map zoom on the chart
                    validate: function() {
                ...
                navModuleFactory([
                    ...
                    // Only run this model if we have range selector
                    validate: function() {
                ...
                navModuleFactory([
                    ...
                    // Only run this model if we have at least one legend - wait for it - item.
            ...
            // Init nav model index. We start at the first model, and as the user navigates through the chart the index will increase to use different handler modules.
